...
trait AnyPipeline { pipeline =>
  ...
  case class UmiAnalysisConfig(val size: Int) extends AnyUmiAnalysisConfig with CommonConfigDefaults
  case class AnnotationConfig(val size: Int) extends AnyAnnotationConfig with CommonConfigDefaults
  // ...
  case class AllInOneConfig(val size: Int) extends AnyAllInOneConfig with CommonConfigDefaults
  ...
  private def outputFor(sampleId: SampleID, stepConfig: AnyStepConfig)(d: AnyData): (AnyData, S3Resource) = {
    d -> S3Resource( outputS3Folder(sampleId, stepConfig.stepName) / s"${sampleId}_${d.label}" )
  }
  ...
  lazy val annotationDataMappings: DataMappings[igblastAnnotation.TRB.type] = inputPairedReads.toList.map { case (sampleId, (r1, r2)) =>
  ...
  trait FixedName extends AnyLoquat { override lazy val fullName: String = s"${pipeline.fullName}.${this.toString}" }

  case object umiAnalysisStep extends Loquat(umiAnalysisConfig, umiAnalysis.dataProcessing)(umiAnalysisDataMappings) with FixedName
  case object annotationStep  extends Loquat(annotationConfig, igblastAnnotation.TRB)(annotationDataMappings) with FixedName
  // ...
  case object allInOneStep  extends Loquat(allInOneConfig, allInOne.dataProcessing)(allInOneDataMappings) with FixedName
}
